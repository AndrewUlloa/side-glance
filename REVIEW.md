...
| UX and accessibility | Pass in production | The current Vercel production deployment was checked at the annotated 1256×833 viewport: the simplified hero has no eyebrow or proof strip, no horizontal overflow, and no browser errors. Earlier desktop and 390×844 mobile interaction checks remain green. Install commands remain explicitly labeled unavailable until the beta is published. |
...
| Motion and interaction | Pass in production | A finite, stage-driven storyboard now starts with four concurrent terminal sessions and resolves them into a cool-to-urgent cyan/green/amber/red stack with Replay. Desktop, 390×844 mobile, keyboard controls, the moved interactive playground, and fresh-session reduced motion pass with no overflow or runtime errors; reduced motion hydrates without mismatch, shows all four awake terminals, and runs zero animations. |
...
| Preview annotation tooling | Pass locally and on Preview | Agentation 3.0.2 is pinned as a development dependency and loaded only after hydration when the centralized environment gate enables it. Local development visibly shows the toolbar; protected Preview `dpl_D8TxW8oebtacgRantEBqPgoQjqmn` returns HTTP 200, embeds `enabled: true`, and omits the reviewed eyebrow and proof strip. Production-mode and live-production browsers show no toolbar and zero Agentation requests. No endpoint, MCP mutation, or remote sync is configured. The production HTML test also inspects every initially loaded script for Agentation implementation markers. |
