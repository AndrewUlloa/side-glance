...
| Preview annotation tooling | Pass locally; preview pending | Agentation 3.0.2 is pinned as a development dependency and loaded only after hydration when the centralized environment gate enables it. Local development visibly shows the toolbar; a production-mode browser shows no toolbar and zero Agentation requests. No endpoint, MCP mutation, or remote sync is configured. The production HTML test also inspects every initially loaded script for Agentation implementation markers. |
