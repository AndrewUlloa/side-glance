...
test("release validation accepts only Side Glance's public protected matching version tag", async (context) => {
  ...
  const base = {
    ...
    GITHUB_REF_TYPE: "tag",
    GITHUB_REF_NAME: "v0.1.0-beta.11",
  ...
  assert.match(fields, /^version=0\.1\.0-beta\.11$/mu);
...
test("release channels allow retries and upgrades but reject backward dist-tag moves", async () => {
  await command(process.execPath, [channelValidator, "0.1.0-beta.11", "beta", "0.1.0-beta.10"]);
  await command(process.execPath, [channelValidator, "0.1.0-beta.11", "beta", "0.1.0-beta.11"]);
