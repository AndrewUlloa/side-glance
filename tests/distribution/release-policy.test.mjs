...
test("release validation accepts only Side Glance's public protected matching version tag", async (context) => {
  ...
  assert.match(fields, /^version=0\.1\.0-beta\.4$/mu);
...
test("release channels allow retries and upgrades but reject backward dist-tag moves", async () => {
  await command(process.execPath, [channelValidator, "0.1.0-beta.4", "beta", "0.1.0-beta.3"]);
  await command(process.execPath, [channelValidator, "0.1.0-beta.4", "beta", "0.1.0-beta.4"]);
