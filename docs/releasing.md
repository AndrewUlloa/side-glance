...
Signal releases are assembled once from a protected version tag. The exact npm tarball and native archives are tested, checksummed, attested, staged as a draft GitHub release, published to npm, and only then made visible. Homebrew tap updates remain a separate credential boundary.
...
After the release finishes, download the public artifacts again, verify `SHA256SUMS`, execute the Linux x64 binary, compare npm integrity, and open a separate Homebrew tap pull request using the generated `terminal-signal.rb`.
