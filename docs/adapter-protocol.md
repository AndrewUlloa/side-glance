...
The wrapper exports `SIDE_GLANCE_SURFACE_ID`, `SIDE_GLANCE_SESSION_ID`, and, when available, `SIDE_GLANCE_TTY` and `SIDE_GLANCE_TMUX_PANE`. `--label` also exports `SIDE_GLANCE_LABEL`; `--notification-sound` exports `SIDE_GLANCE_NOTIFICATION_SOUND`; `--terminal-title` exports `SIDE_GLANCE_TERMINAL_TITLE=1`. Explicit verified values win. Otherwise Side Glance uses inherited tmux identity, checks the hook's standard input with `tty`, then performs a bounded process ancestry lookup for a canonical TTY candidate. The ancestry records must remain stable and owned by the current user; the renderer then requires an owned character device and rechecks its identity when opened before treating it as a verified TTY or writing OSC bytes. This makes plain `claude`, `codex`, and experimental `gemini` the primary supported local CLI path, but discovery is not guaranteed: desktop applications, detached processes, and remote app servers can remain targetless. An installed managed hook that receives no safe target still records and acknowledges its event so the provider UI is never polluted by a Side Glance discovery failure. `side-glance run` remains the explicit fallback for a stable surface identity, private label, or process-exit boundary. A manually invoked non-notification hook remains strict and requires `--surface` or a discoverable TTY.
